...
class AccountInvoice(orm.Model):
    ...
    def einvoice_type_selection(self, cr, uid, valstype, partner_vat,
                                amount=None, context=None):
        einv_type_model = self.pool['italy.ade.invoice.type']
        scope = 'NN'
        if partner_vat:
    ...
    def onchange_set_einvoice_type(self, cr, uid, ids, partner_id, type,
                                   amount_total, einvoice_type_selection,
                                   nvoice_type_id, context=None):
        res = {'value': {}
        partner_model = self.pool.get('res.partner')
        vat = None
        if partner_id:
            vat = partner_model.browse(
                cr, uid, partner.id).vat
        if partner_id and vat:
            ids = einvoice_type_selection(
                cr, uid, type, vat, amount_total)
        else:
            ids = einvoice_type_selection(
                cr, uid, type, 'IT', amount_total)
        if not ids:
            res['value']['invoice_type_id'] = False
        elif (not invoice_type_id  or invoice_type_id not in ids):
            res['value']['invoice_type_id'] = ids[0]
        if not res['value']:
            res = {}\
        return res
