...
class OpenItems(object):
    ...
    def _set_odoo_env(self, inv_line=None):
        ...
        if (
                ...
                and inv_line.account_id.user_type_id == self.env.ref(
    ...
    def add_move_line(self, move_line):
        ...
        if move_line.move_id.journal_id.type in ("sale", "sale_refund"):
            ...
            self.amount += balance
        if (
                move_line.account_id.user_type_id in self.valid_account_types
                and (not move_line.partner_id
            ...
            if key not in self.account_ids:
                self.account_ids[key] = {
                    "debit": 0.0, "credit": 0.0, "payorder_lines": set()
                }
    ...
    def add_payorder_line(self, payorder_line, force=True):
        ...
        if (
                payorder_line not in self.payorder_lines
                and (force
                     or payorder_line.partner_id in list(self.partner_amounts.keys()))
        ):
    ...
    def load_move_vals(self, move_date=None, expenses_amount=0.0):
        ...
        for (account, partner) in self.account_ids.keys():
            ...
            if (
                    self.odoo_version == 12
                    and self.account_ids[key]["payorder_lines"]
            ):
